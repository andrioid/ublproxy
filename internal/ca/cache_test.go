...
func TestGetPortalCertCachesAndReturns(t *testing.T) {
	caCert, caKey, err := Generate()
	if err != nil {
		t.Fatal(err)
	}
	cache := NewCache(caCert, caKey)
	cache.SetPortalParams("localhost", nil)

	cert1, err := cache.GetPortalCert()
	if err != nil {
		t.Fatal(err)
	}

	cert2, err := cache.GetPortalCert()
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetPortalCertRegeneratesExpired(t *testing.T) {
	caCert, caKey, err := Generate()
	if err != nil {
		t.Fatal(err)
	}
	...
	// Manually expire the cached portal entry
	cache.mu.Lock()
	cache.portalCert.expiresAt = time.Now().Add(-1 * time.Hour)
	cache.mu.Unlock()
